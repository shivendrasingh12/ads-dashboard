...
function getCustomer() {
  const opts = { customer_id: process.env.GOOGLE_CUSTOMER_ID, refresh_token: process.env.GOOGLE_REFRESH_TOKEN }
  if (process.env.GOOGLE_LOGIN_CUSTOMER_ID) opts.login_customer_id = process.env.GOOGLE_LOGIN_CUSTOMER_ID
  return getClient().Customer(opts)
}

export default async function handler(req, res) {
  const { dateFrom, dateTo, level = 'campaigns', campaignId, adGroupId } = req.query
  if (!isGoogleConfigured()) return res.status(200).json({ error: 'Google Ads not configured.' })
  const from = dateFrom || new Date(Date.now() - 30 * 86400000).toISOString().slice(0, 10)
  const to = dateTo || new Date().toISOString().slice(0, 10)

  try {
    if (level === 'campaigns') {
      const all = await fetchGoogleCampaigns(from, to)
      const campaigns = all.filter(c => c.spend > 0 || c.impressions > 0).map(c => ({
        id: c.id, name: c.name, status: c.status, type: 'MULTI_CHANNEL',
        spend: c.spend, impressions: c.impressions, clicks: c.clicks,
        conversions: c.conversions, ctr: c.ctr, cpc: c.cpc, cpa: c.cpa, videoViews: 0
      }))
    ...
    if (level === 'adgroups') {
      ...
      const all = await fetchGoogleAdGroups(from, to)
      const adgroups = all.filter(ag => ag.campaignId === campaignId && (ag.spend > 0 || ag.impressions > 0)).map(ag => ({
        id: ag.id, name: ag.name, status: ag.status,
        spend: ag.spend, impressions: ag.impressions, clicks: ag.clicks,
        conversions: ag.conversions, ctr: ag.ctr, cpc: ag.cpc, cpa: ag.cpa, videoViews: 0
      }))
    ...
    if (level === 'assets') {
      ...
      try {
        ...
        `)
      } catch (e) { console.error('Asset view query error:', e.message) }
        ...
        if (!ads[id]) {
          ...
          ads[id] = {
            ...
            youtubeVideos: (ad.app_ad?.youtube_videos || []).map(v => {
              ...
              return { videoId: vid, thumbnailUrl: vid ? 'https://img.youtube.com/vi/' + vid + '/mqdefault.jpg' : null }
            }).filter(v => v.videoId),
    ...
    return res.status(200).json({ error: 'Google Ads error: ' + e.message })
  }
}
