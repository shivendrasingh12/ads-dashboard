...
const NETWORKS_LIST = ['Search', 'YouTube', 'Display', 'Search Partner']
...
NETWORKS: ${row.networkBreakdown.filter(n => n.metrics.spends > 0).map(n => `${n.network}: Spend ₹${n.metrics.spends}, CTR ${n.metrics.ctr}%, IRate ${n.metrics.iRate}%, Custs ${n.metrics.customers}, CAC ₹${n.metrics.cac}`).join(' | ')}
ADGROUPS (top 6 by spend): ${row.adgroupBreakdown.filter(a => a.metrics.spends > 0).slice(0, 6).map(a => `${a.adGroupName}: ₹${a.metrics.spends}, CTR ${a.metrics.ctr}%, IRate ${a.metrics.iRate}%, Custs ${a.metrics.customers}, Rating(CTR:${a.ratings.ctr},IRate:${a.ratings.iRate},CAC:${a.ratings.cac})`).join(' | ')}`
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.6fr', borderTop: '0.5px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ padding: '8px 14px 4px', fontSize: 10, fontWeight: 600, color: '#534AB7', textTransform: 'uppercase', letterSpacing: '.05em', borderBottom: '0.5px solid var(--border)' }}>
                Adgroups · {agRows.length} active
              </div>
              <div style={{ overflowX: 'auto', flex: 1 }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                  <thead>
                    <tr style={{ background: 'var(--bg2)' }}>
                      {['Adgroup','Spend+GST','CTR','I Rate','Regs','Custs','CAC+GST','R2C'].map(h => (
...
export default function UACFunnelView({ filters, sheetId, title }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 500 }}>Networks:</span>
        {NETWORKS_LIST.map(n => (
          <div key={n} onClick={() => toggleNetwork(n)}
            style={{ fontSize: 11, padding: '4px 8px', borderRadius: 5, cursor: 'pointer', border: '0.5px solid', borderColor: selectedNetworks.includes(n) ? '#378ADD' : 'var(--border)', background: selectedNetworks.includes(n) ? '#E6F1FB' : 'transparent', color: selectedNetworks.includes(n) ? '#0C447C' : 'var(--text3)', userSelect: 'none' }}>
            {n}
          </div>
        ))}
      ...
      {loading && <div style={{ padding: 32, textAlign: 'center', color: 'var(--text3)', fontSize: 13 }}>Loading {title || 'UAC funnel'}...</div>}
      ...
      {!loading && data && (
        <div style={{ border: '0.5px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '7px 14px', fontSize: 10, color: 'var(--text3)', borderTop: '0.5px solid var(--border)' }}>
            Google Sheets · ↑↓ = % vs prior · click city to expand · zero-spend adgroups hidden · ▲▼ = relative to peers
          </div>
        </div>
      )}
    </div>
